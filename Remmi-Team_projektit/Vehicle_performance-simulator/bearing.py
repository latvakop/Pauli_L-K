...
class Bearing:
    ...
    def calculate_loss(self, VehicleSpeed, F_a, F_r):
        ...
        phi_rs = 1/math.exp((6*10**-8)*self.OilOperatingVisc*WheelRpm*(self.BearingInnerDia+self.BearingOuterDia)*math.sqrt(self.K_z/(2*(self.BearingOuterDia-self.BearingInnerDia))))
        if self.BearingType == "Angular":
            ...
            if F_a == 0:
                ...
                G_rr = self.R_1*self.BearingMeanDia**1.96*(F_r+self.R_2/math.sin(math.radians(alpha_F))*F_a)**0.54
        ...
        if self.BearingType == "Angular":
            ...
            if F_a == 0:
                ...
                alpha_F = 24.6*(F_a/self.C_0)**0.24
                G_sl = self.S_1*self.BearingMeanDia**-0.145*(F_r**5+(self.S_2*self.BearingMeanDia**1.5)/math.sin(math.radians(alpha_F))*F_a**4)**(1/3)
